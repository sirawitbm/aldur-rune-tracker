...
class MapChangePopup(QWidget):
    """Appears when the log watcher detects a likely new map entry."""
    ...
    def __init__(self, timeout_sec: float):
        ...
        reset_btn.setStyleSheet(
            "QPushButton { background: rgba(120,20,20,230); color: white; border-radius: 4px; padding: 4px 10px; }"
            "QPushButton:hover { background: rgba(160,30,30,240); }"
        )
        reset_btn.clicked.connect(self._on_reset)

        keep_btn = QPushButton("Keep tracking")
        keep_btn.setStyleSheet(
            "QPushButton { background: rgba(40,40,40,230); color: white; border-radius: 4px; padding: 4px 10px; }"
            "QPushButton:hover { background: rgba(70,70,70,240); }"
        )
        ...
        container = QWidget()
        container.setStyleSheet("background: rgba(15,15,15,230); border-radius: 6px;")
        inner = QVBoxLayout(container)
        ...
        self._timer = QTimer(self)
        self._timer.setSingleShot(True)
        self._timer.timeout.connect(self._on_keep)
    ...
    def announce(self, area_name: str):
        self.label.setText(f"Entered: {area_name}\nReset rune tracker for the new map?")
        self._position()
        self.show()
        self.raise_()
        self._timer.start(int(self.timeout_sec * 1000))
